document.addEventListener("DOMContentLoaded", function () {
  ...
  // ==========================
  // Al hacer clic en el botón "Agregar Producto" (con clase "noselect")
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  folders.forEach(folder => {
      folder.addEventListener('click', async function() {
          ...
          try {
              const response = await fetch(`../controllers/AccionProductos.php?category=${encodeURIComponent(category)}`);
              const products = await response.json();
              ...
              if (products.length > 0) {
                  ...
                  productTableBody.innerHTML = '<tr><td colspan="3">No hay productos disponibles</td></tr>';
              }
          } catch (error) {
              console.error('Error al cargar los productos:', error);
              productTableBody.innerHTML = '<tr><td colspan="3">Error al cargar los productos</td></tr>';
          }
      });
  });
